...
<template functional>
  <Submenu :key="props.menu.name" :name="props.menu.name">
    <template slot="title">
      <Icon v-if="parent.getMenuIcon(props.menu)" :type="parent.getMenuIcon(props.menu)" />
    ...
    <template v-for="item in parent.getSubMenus(props.menu)">
      <template v-if="parent.showMenu(item)">
        <MenuItem v-if="!parent.getSubMenus(item)" :key="item.name" :name="item.name">
          <Icon v-if="parent.getMenuIcon(item)" :type="parent.getMenuIcon(item)" />
